...
class Ligand:
    def __init__(
        ...
        if isinstance(atom_positions, list):
            ...
            if conformer_axis in [0, None]:
                self.atom_positions = np.ascontiguousarray(
                    np.moveaxis(self.atom_positions, 0, 1)
                )

        assert self.num_atoms == self.atom_positions.shape[0]
        self.num_conformers: int = self.atom_positions.shape[1]

        self.pharmacophore_nodes: dict[str, list[PharmacophoreNode]] = (
            get_pharmacophore_nodes(self.pbmol)
        )
    ...
    def load_from_file(cls, filename: os.PathLike) -> Ligand:
        ...
        pbmol_list: list[pybel.Molecule] = list(
            pybel.readfile(extension[1:], str(filename))
        )
...
class LigandGraph:
    ...
    def __group_nodes(self, obj):
        ...
        for node in self.nodes:
            ...
            if (
                "HBond_acceptor" in pharmacophore_types
                or "HBond_donor" in pharmacophore_types
            ):
        ...
        index_to_node = {
            next(iter(node.atom_indices)): node for node in hydrophobic_nodes
        }
...
class LigandNode:
    ...
    def add_neighbors(self, neighbor: LigandNode) -> LigandEdge:
        ...
        if check_type(node_types, "Hydrophobic") and check_type(
            neighbor_node_types, "Aromatic"
        ):
            if self.atom_indices.issubset(neighbor.atom_indices):
                self.dependence_nodes.add(neighbor)
        elif check_type(node_types, "Aromatic") and check_type(
            neighbor_node_types, "Hydrophobic"
        ):
