...
class PharmacoNet():
    ...
    def run_gui(
        ...
        pdbblock, protein_image, non_protein_area, token_positions, tokens = self.__parse_protein(protein_pdb_path, center)
        ...
        if density_maps is not None:
            x, y, z = center.tolist()
            pharmacophore_model = PharmacophoreModel.create(pdbblock, (x, y, z), self.out_resolution, self.out_size, density_maps)
    ...
    def __create_density_maps_gui(
        ...
        non_protein_area = non_protein_area.to(dtype=torch.bool) if non_protein_area is not None else None
        ...
        for idx in range(0, hotspots.size(0)):
            ...
            _hotspots, _hotspot_features = hotspots[idx:idx + 1], hotspot_features[idx:idx + 1]
        ...
        if thread.force_stop:
            return
        thread.progress_updated.emit(f'Export...', 100,)
        out = []
